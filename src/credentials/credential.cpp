...
constexpr std::array<uint8_t, 8> STORE_MAGIC{ 'V', 'F', 'I', 'D',
											  'O', '2', 'D', 'B' };
constexpr uint8_t STORE_FORMAT_VERSION = 1;
constexpr std::size_t GENERATION_SIZE  = 8;
constexpr std::size_t NONCE_SIZE	   = 12;
constexpr std::size_t TAG_SIZE		   = 16;
constexpr std::size_t HEADER_SIZE = STORE_MAGIC.size() + 1 + GENERATION_SIZE + NONCE_SIZE;
...
[[noreturn]] void throw_system_error(int error, std::string operation) {
	throw std::system_error(error, std::generic_category(), std::move(operation));
}
...
void write_all(int fd, std::span<const uint8_t> bytes) {
	...
	while(offset < bytes.size()) {
		...
		const ssize_t written = ::write(fd, bytes.data() + offset, chunk_size);
		if(written == -1) {
			const int error = errno;
			if(error == EINTR) {
				continue;
		...
		if(written == 0) {
			throw std::runtime_error("write temporary credential store made no "
									 "progress");
		}
...
std::vector<uint8_t> read_all(int fd, std::size_t size) {
	...
	while(offset < contents.size()) {
		...
		if(count == 0) {
			throw std::runtime_error("Credential store was truncated while "
									 "reading");
		}
...
std::pair<UniqueFd, std::string> create_temporary_file(int directory_fd) {
	...
	for(int attempt = 0; attempt < maximum_attempts; ++attempt) {
		...
		const int fd =
			::openat(directory_fd, name.c_str(), O_WRONLY | O_CREAT | O_EXCL | O_CLOEXEC | O_NOFOLLOW, S_IRUSR | S_IWUSR);
		if(fd >= 0) {
	...
	throw std::runtime_error("Could not create a unique temporary credential "
							 "store");
}

void validate_credential(const StoredCredential& credential) {
	if(credential.id.size() < 16 || credential.id.size() > 1024) {
		throw std::invalid_argument("Credential ID must contain between 16 and "
									"1024 bytes");
	}
	if(credential.rpId.empty() || credential.rpId.size() > 253) {
		throw std::invalid_argument("Relying Party ID must contain between 1 "
									"and 253 bytes");
	}
	if(credential.userId.empty() || credential.userId.size() > 64) {
		throw std::invalid_argument("User ID must contain between 1 and 64 "
									"bytes");
	}
	if(credential.alg != -7) {
	...
	if(credential.public_blob.empty() || credential.public_blob.size() > MAX_CREDENTIAL_BLOB_SIZE) {
...
std::filesystem::path prepare_store_directory(const std::filesystem::path& store_path) {
...
UniqueFd open_store_directory(const std::filesystem::path& directory) {
	...
	if(fd == -1) {
		const int error = errno;
		throw_system_error(error, "open credential store directory " + directory.string());
	}
	...
	if(status.st_uid != ::geteuid()) {
		::close(fd);
		throw std::runtime_error("Credential store directory must be owned by "
								 "the service user");
	}
	if((status.st_mode & 0777) != 0700) {
		::close(fd);
		throw std::runtime_error("Credential store directory permissions must "
								 "be 0700");
	}
...
std::optional<std::vector<uint8_t>> read_store_file(const std::filesystem::path& store_path) {
	const auto filename = store_path.filename();
	if(filename.empty() || filename == "." || filename == "..") {
		throw std::invalid_argument("Credential store path must include a "
									"valid filename");
	}
	...
	const int file_fd =
		::openat(directory.get(), filename.c_str(), O_RDONLY | O_CLOEXEC | O_NOFOLLOW);
	...
	if(!S_ISREG(status.st_mode) || status.st_nlink != 1) {
		throw std::runtime_error("Credential store must be a regular file with "
								 "one link");
	}
	if(status.st_uid != ::geteuid()) {
		throw std::runtime_error("Credential store must be owned by the "
								 "service user");
	}
	if((status.st_mode & 0777) != 0600) {
		throw std::runtime_error("Credential store permissions must be 0600");
	}
	if(status.st_size < 0 || static_cast<uintmax_t>(status.st_size) > MAX_STORE_SIZE) {
...
void atomic_write_file(const std::filesystem::path& store_path, std::span<const uint8_t> contents) {
	const auto filename = store_path.filename();
	if(filename.empty() || filename == "." || filename == "..") {
		throw std::invalid_argument("Credential store path must include a "
									"valid filename");
...
std::vector<uint8_t> read_byte_array(const nlohmann::json& entry, std::string_view field_name) {
	...
	if(!value.is_array()) {
		throw std::runtime_error(std::string(field_name) + " is not a byte array");
	}
	if(value.size() > MAX_CREDENTIAL_BLOB_SIZE) {
		throw std::runtime_error(std::string(field_name) + " exceeds the size limit");
	}
	...
	for(const auto& element : value) {
		if(!element.is_number_unsigned()) {
			throw std::runtime_error(std::string(field_name) + " contains a value that is not an unsigned integer");
		}

		const auto number = element.get<uint64_t>();
		if(number > std::numeric_limits<uint8_t>::max()) {
			throw std::runtime_error(std::string(field_name) + " contains a value above 255");
		}
...
template <typename Container> class CleanseBuffer {
	public:
...
CredentialStoreLock::CredentialStoreLock(const std::filesystem::path& store_path) {
	const auto directory_path = prepare_store_directory(store_path);
	auto directory			  = open_store_directory(directory_path);
...
CredentialStore::CredentialStore(std::filesystem::path path, Key key, StoreGenerationCounter* generation_counter)
: storePath_(std::move(path)), storeKey_(std::move(key)),
...
// Cryptography
CredentialStore::DecryptedStore
CredentialStore::decrypt(const std::vector<uint8_t>& ciphertext) const {
	...
	const uint64_t generation			= read_uint64_be(
		  std::span<const uint8_t, GENERATION_SIZE>(ciphertext.data() + generation_offset, GENERATION_SIZE)
	  );
	const uint8_t* iv = ciphertext.data() + generation_offset + GENERATION_SIZE;
	const uint8_t* cipher = ciphertext.data() + HEADER_SIZE;
	const uint8_t* tag	  = ciphertext.data() + ciphertext.size() - TAG_SIZE;
	const int ctlen =
		openssl_checked_size(ciphertext.size() - HEADER_SIZE - TAG_SIZE, "credential ciphertext");

	auto context = openssl_make_cipher_context();
	openssl_check(EVP_DecryptInit_ex(context.get(), EVP_aes_256_gcm(), nullptr, nullptr, nullptr), "EVP_DecryptInit_ex");
	openssl_check(EVP_CIPHER_CTX_ctrl(context.get(), EVP_CTRL_GCM_SET_IVLEN, NONCE_SIZE, nullptr), "EVP_CTRL_GCM_SET_IVLEN");
	openssl_check(EVP_DecryptInit_ex(context.get(), nullptr, nullptr, storeKey_.data(), iv), "EVP_DecryptInit_ex key and IV");
	openssl_check(
		EVP_CIPHER_CTX_ctrl(context.get(), EVP_CTRL_GCM_SET_TAG, TAG_SIZE, const_cast<uint8_t*>(tag)),
		"EVP_CTRL_GCM_SET_TAG"
	);

	std::vector<uint8_t> plain(static_cast<std::size_t>(ctlen) + EVP_MAX_BLOCK_LENGTH);
	int outl = 0;
	openssl_check(
		EVP_DecryptUpdate(context.get(), nullptr, &outl, ciphertext.data(), openssl_checked_size(HEADER_SIZE, "credential store header")),
		"EVP_DecryptUpdate AAD"
	);
	openssl_check(EVP_DecryptUpdate(context.get(), plain.data(), &outl, cipher, ctlen), "EVP_DecryptUpdate");
	int finlen = 0;
	openssl_check(EVP_DecryptFinal_ex(context.get(), plain.data() + outl, &finlen), "EVP_DecryptFinal_ex (GCM authentication failed)");

	plain.resize(static_cast<std::size_t>(outl + finlen));
	return DecryptedStore{ .generation = generation, .plaintext = std::move(plain) };
}

std::vector<uint8_t>
CredentialStore::encrypt(const std::vector<uint8_t>& plaintext, uint64_t generation) const {
	...
	auto context = openssl_make_cipher_context();
	openssl_check(EVP_EncryptInit_ex(context.get(), EVP_aes_256_gcm(), nullptr, nullptr, nullptr), "EVP_EncryptInit_ex");
	openssl_check(EVP_CIPHER_CTX_ctrl(context.get(), EVP_CTRL_GCM_SET_IVLEN, NONCE_SIZE, nullptr), "EVP_CTRL_GCM_SET_IVLEN");
	openssl_check(EVP_EncryptInit_ex(context.get(), nullptr, nullptr, storeKey_.data(), iv.data()), "EVP_EncryptInit_ex key and IV");

	int outl = 0;
	openssl_check(
		EVP_EncryptUpdate(context.get(), nullptr, &outl, out.data(), openssl_checked_size(out.size(), "credential store header")),
	...
	int final_len = 0;
	openssl_check(EVP_EncryptFinal_ex(context.get(), cipher.data() + outl, &final_len), "EVP_EncryptFinal_ex");

	std::array<uint8_t, TAG_SIZE> tag{};
	openssl_check(EVP_CIPHER_CTX_ctrl(context.get(), EVP_CTRL_GCM_GET_TAG, TAG_SIZE, tag.data()), "EVP_CTRL_GCM_GET_TAG");
...
void CredentialStore::save_storage(const Storage& storage) {
	if(requiresReload_) {
		throw std::runtime_error("Credential store must be reloaded after an "
								 "interrupted commit");
	}
	...
	if(generationCounter_ != nullptr) {
		...
		if(current_generation != generation_) {
			throw std::runtime_error("Credential store generation changed "
									 "unexpectedly");
		}
	}
	...
	if(generationCounter_ != nullptr) {
		try {
			...
			if(generationCounter_->read() != next_generation) {
				throw std::runtime_error("Credential store counter did not "
...
CredentialStore::Storage CredentialStore::parse_storage(const nlohmann::json& json) const {
	if(!json.is_array())
		throw std::runtime_error("Storage is not an array");
	if(json.size() > MAX_CREDENTIALS) {
		throw std::runtime_error("Credential store contains too many "
								 "credentials");
	}
	Storage loaded;
	...
	for(const auto& entry : json) {
		...
		if(cred.alg != -7)
			throw std::runtime_error("Invalid encryption alrogithm");

		const auto& signCountValue = entry.at("signCount");
		if(!signCountValue.is_number_unsigned())
			throw std::runtime_error("Invalid Sign Count type");

		const auto count = signCountValue.get<uint64_t>();
		if(count > std::numeric_limits<uint32_t>::max())
			throw std::runtime_error("Sign Count out of range");
...
void CredentialStore::load() {
	...
	if(!encrypted) {
		if(generationCounter_ != nullptr && generationCounter_->read() != 0) {
			throw std::runtime_error("Credential store is missing but its "
									 "rollback counter is not zero");
		}
		stored_.clear();
		generation_		= 0;
		requiresReload_ = false;
	...
	json j = json::parse(decrypted.plaintext.begin(), decrypted.plaintext.end());

	auto loaded = parse_storage(j);

	if(generationCounter_ != nullptr && decrypted.generation == counter_generation + 1) {
		generationCounter_->increment();
		if(generationCounter_->read() != decrypted.generation) {
			throw std::runtime_error("Credential store crash recovery counter "
									 "verification failed");
		}
	}
...
void CredentialStore::put(const StoredCredential& cred, uint32_t owner_uid) {
	require_ready();
...
std::vector<StoredCredential> CredentialStore::find_for_assertion(
	...
	for(const auto& [credential_id, credential] : stored_) {
		(void)credential_id;
		if(credential.ownerUid == owner_uid && credential.discoverable &&
...
[[nodiscard]] std::vector<CredentialSummary> CredentialStore::list_credentials() const {
	...
	std::sort(public_credential_data.begin(), public_credential_data.end(), [](const auto& left, const auto& right) {
